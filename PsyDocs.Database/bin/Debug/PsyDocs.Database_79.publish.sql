...
--END
  ...
  INSERT INTO [dbo].[TypeSection]
			...
			,[Nom]
			,[Description])
  SELECT NEWID(),'Historique de travail' as ht,'Description de l’historique de travail du client'
  where NOT EXISTS
   (SELECT 0 
    FROM   TypeSection as t2
    WHERE  t2.Nom = Nom) 				
  SELECT NEWID(),'Historique conjugal' as ht,'Description de l’historique conjugal du client'
  where NOT EXISTS
   (SELECT 0 
    FROM   TypeSection as t2
    WHERE  t2.Nom = Nom)
...
--   WHERE TD.Nom = 'Note d’évolution' AND (TS.Nom = 'Autre' OR TS.Nom = 'Recommandations')
--      or TD.Nom = 'Note d’évolution' AND (TS.Nom = 'Autre' OR TS.Nom = 'Recommandations')
--   AND NOT EXISTS
